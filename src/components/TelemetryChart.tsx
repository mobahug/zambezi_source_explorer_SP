...
const TelemetryChart = ({ data }: TelemetryChartProps) => {
  ...
  useEffect(() => {
    ...
    const yDomainHr: [number, number] = [Math.min(hrExtent[0] ?? 80, 80), Math.max(hrExtent[1] ?? 160, 160)];
    const yDomainPh: [number, number] = [Math.min(phExtent[0] ?? 6.6, 6.4), Math.max(phExtent[1] ?? 7.4, 7.6)];

    const xScale = d3.scaleLinear().domain(xDomain).range([0, innerWidth]);
    const yScaleHr = d3.scaleLinear().domain(yDomainHr).nice().range([innerHeight, 0]);
    const yScalePh = d3.scaleLinear().domain(yDomainPh).range([innerHeight, 0]);

    const xAxis = d3.axisBottom(xScale).ticks(5).tickFormat((d) => `${d}s`);
    const yAxis = d3
      .axisLeft(yScaleHr)
      .ticks(4)
      .tickSize(-innerWidth)
  ...
  return <svg ref={svgRef} role="img" aria-label="Telemetry chart" width="100%" height={240} />;
};
